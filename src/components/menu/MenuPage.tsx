...
export function MenuPage({ catalog, isPreviewMode }: MenuPageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#f6fbf7] text-[#233224]">
      <section
        className="overflow-hidden border-b border-[#dbe8dd] bg-[#ffffff]"
      >
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-5 py-8 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#dbe8dd] bg-white px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#2f6b46]">
              <ScanLine size={14} />
              Online Menu
            </div>
            <h1 className="text-4xl font-black leading-[0.95] text-[#173f2a] sm:text-6xl">
              <span className="mb-2 block">
                <PurivaLogo className="text-3xl sm:text-4xl" />
              </span>
              Live
              <span className="block text-[#2f6b46]">Cold Pressed Juice Menu</span>
            </h1>
      ...
      {isPreviewMode ? (
        <div className="mx-auto w-full max-w-6xl px-5 pt-4 sm:px-8">
          <div className="rounded-[8px] border border-[#dbe8dd] bg-white px-4 py-3 text-sm text-[#2f6b46]">
              ...
              <button
                ...
                className={`inline-flex h-12 shrink-0 items-center gap-2 rounded-[8px] border px-4 text-sm font-black uppercase transition duration-200 hover:-translate-y-0.5 hover:shadow-sm ${
                  isActive
                    ...
                    : "border-[#dbe8dd] bg-white text-[#4d5a47]"
                }`}
              >
                <Icon size={17} />
...
function MenuImageFrame({
  ...
  return (
    <div className={`${className} aspect-[4/3] overflow-hidden rounded-[8px] border border-[#dbe8dd] bg-[#f6fbf7]`}>
      {imageUrl ? (
        <div
          aria-label={name}
        ...
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-4 text-center">
          ...
          <p className="text-xs font-black uppercase tracking-[0.12em] text-[#2f6b46]">Photo coming soon</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <section>
      <div className="mb-4 flex items-end justify-between gap-4">
        ...
        {section.priceIdr ? (
          <div className="shrink-0 text-xl font-black text-[#2f8f5b]">{formatShortIdr(section.priceIdr)}</div>
        ) : null}
      </div>
      ...
      {section.displayMode === "recipe-cards" ? (
        <div className="grid gap-3 sm:grid-cols-2">
          {section.entries.map((entry) => (
            <details key={entry.id} className="group rounded-[8px] border border-[#dbe8dd] bg-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md">
              <summary className="relative cursor-pointer list-none p-4 pr-12">
                <div>
                  ...
                  {entry.benefit ? <p className="mt-1 text-sm font-semibold text-[#2f6b46]">{entry.benefit}</p> : null}
                </div>
                <ChevronDown className="absolute right-4 top-4 shrink-0 text-[#2f6b46] transition-transform group-open:rotate-180" size={18} />
              </summary>
              <div className="animate-menu-panel border-t border-[#dbe8dd] px-4 pb-4 pt-3">
                <p className="text-xs font-black uppercase tracking-[0.14em] text-[#2f8f5b]">Ingredients</p>
        ...
        <>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {section.entries.map((entry) => (
              <div
                key={entry.id}
                className="rounded-[8px] border border-[#dbe8dd] bg-white p-3 text-sm font-black uppercase text-[#233224] shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md"
              >
                <MenuImageFrame imageUrl={entry.imageUrl} name={entry.name} accentColor={entry.accentColor} />
          ...
          {section.slug === "cut-fruits" || section.slug === "blended-juice" ? (
            <details className="group mt-3 rounded-[8px] border border-[#dbe8dd] bg-white px-4 py-3 shadow-sm">
  ...
  return (
    <section>
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        {section.priceIdr ? (
          <div className="text-2xl font-black text-[#2f8f5b]">{formatShortIdr(section.priceIdr)}</div>
        ) : null}
      </div>
      ...
      <div className="space-y-4">
        {categories.map((category, index) => {
          ...
          return (
            <article key={category.slug} className="rounded-[8px] border border-[#dbe8dd] bg-white shadow-sm">
              <div className="border-b border-[#dbe8dd] px-4 py-4">
              ...
              <div className="divide-y divide-[#dbe8dd]">
                  ...
                  <details key={group.baseName} className="group px-4 py-4">
                    <summary className="flex cursor-pointer list-none flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        ...
                        {group.benefit ? (
                          <p className="mt-1 text-sm font-bold text-[#2f6b46]">{group.benefit}</p>
                        ) : null}
                      </div>
                      <span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-[#2f8f5b]">
                        Mix pilihan
                    ...
                    <div className="mt-3 grid gap-2">
                      {group.mixes.map((mix) => (
                        <div
                          ...
                          className="grid gap-3 rounded-[8px] border border-[#dbe8dd] bg-[#f6fbf7] p-3 transition duration-200 hover:-translate-y-0.5 hover:bg-white sm:grid-cols-[112px_minmax(0,1fr)]"
                        >
                          <MenuImageFrame
      ...
      <div className="mt-4 rounded-[8px] border border-[#dbe8dd] bg-[#173f2a] px-5 py-4 text-center text-white">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-[#bde5c8]">All variants</p>

...
const chartColors = ["#2f8f5b", "#173f2a", "#d97706", "#be123c", "#65a30d", "#2f6b46"];
...
function MoneyTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ name?: string; value?: number; color?: string }>; label?: string }) {
  ...
  return (
    <div className="rounded-[8px] border border-[#dbe8dd] bg-white px-3 py-2 text-xs shadow-sm">
      {label ? <p className="mb-1 font-black uppercase text-[#173f2a]">{label}</p> : null}
      {payload.map((item) => (
        <p key={item.name} className="font-bold text-[#65705e]">
          <span style={{ color: item.color ?? "#2f8f5b" }}>{item.name}</span>: {formatShortIdr(Number(item.value ?? 0))}
        </p>
      ))}
    </div>
  );
...
function CountTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ name?: string; value?: number; color?: string }>; label?: string }) {
  ...
  return (
    <div className="rounded-[8px] border border-[#dbe8dd] bg-white px-3 py-2 text-xs shadow-sm">
      ...
      {payload.map((item) => (
        <p key={item.name} className="font-bold text-[#65705e]">
          <span style={{ color: item.color ?? "#2f8f5b" }}>{item.name}</span>: {Number(item.value ?? 0)}
        </p>
      ))}
    </div>
  );
...
function EmptyChart({ label }: { label: string }) {
  return (
    <div className="flex h-64 items-center justify-center rounded-[8px] bg-[#f6fbf7] p-6 text-center text-sm font-semibold text-[#65705e]">
      {label}
    </div>
  );
}
...
export function SalesReportPage({ report, selectedDate }: { report: SalesReport; selectedDate: string }) {
  ...
  return (
    <div className="text-[#1f2f22]">
      ...
      <form className="mb-5 grid gap-3 rounded-[8px] border border-[#dbe8dd] bg-white p-4 shadow-sm sm:grid-cols-[220px_auto] sm:items-end">
      ...
      <section className="grid gap-3 md:grid-cols-4">
        <div className="rounded-[8px] border border-[#dbe8dd] bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-black text-[#2f8f5b]">{formatShortIdr(report.paidSalesIdr)}</p>
        </div>
        <div className="rounded-[8px] border border-[#dbe8dd] bg-white p-4 shadow-sm">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#2f6b46]">Gross completed</p>
        ...
        <div className="rounded-[8px] border border-[#dbe8dd] bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-black text-[#2f8f5b]">{formatShortIdr(report.averageOrderValueIdr)}</p>
        </div>
      </section>
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={dailyChart} margin={{ top: 10, right: 18, left: 0, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="paidSalesIdr" name="Paid sales" stroke="#2f8f5b" strokeWidth={3} fill="url(#paidSalesGradient)" activeDot={{ r: 5, fill: "#173f2a" }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          ) : (
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={categoryChart} margin={{ top: 10, right: 18, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fill: "#65705e", fontSize: 12 }} tickLine={false} axisLine={false} />
                  <Tooltip content={<CountTooltip />} />
                  <Bar dataKey="quantity" name="Quantity" fill="#2f8f5b" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ) : (

...
begin
  ...
  loop
    ...
    select
      menu_entries.name,
      menu_sections.slug,
      menu_entries.ingredients,
      coalesce(menu_entries.price_idr, menu_sections.price_idr, 0),
      menu_entries.mix_availability
    into v_menu_name, v_section_slug, v_ingredients, v_unit_price, v_mix_availability
    ...
    if v_section_slug in ('cut-fruits', 'blended-juice') then
      ...
      where menu_sections.slug = 'cold-pressed-juice'
        ...
        and lower(coalesce(menu_entries.base_name, menu_entries.name)) in (
          select lower(ingredient) from unnest(v_ingredients) as ingredient
        )
      limit 1;
